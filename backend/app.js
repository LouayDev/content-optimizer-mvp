...
app.post('/signin', async (req, res) => {
  ...
  try {
    ...
    const checkUserSQL = await fs.readFile('./sql/checkUser.sql', {encoding: 'UTF-8'})
    ...
    const newUserSessionSQL = await fs.readFile('./sql/newUserSession.sql', {encoding: 'UTF-8'})
    const newSessionResult = await client.query(newUserSessionSQL, [sessionId, user_id, newExperationDate()])
